import {
    gotValidators as gotAction,
    processingValidator as processingAction,
    processingValidatorFailed  as failedAction,
    addedValidator as addedAction,
    updatedValidator as updatedAction,
    deletedValidator as deletedAction,
...
const resourceUrl = process.env.REACT_APP_API + 'api/validators/';
const odataResourceUrl = process.env.REACT_APP_API + 'odata/validators/';
...
const myState = (state) => { return state.validator }
...
const fetchObjects = (data) => dispatch => {
    ...
    return fetch(finalUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(gotAction(json)); dispatch(fetchStreamsIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const fetchValidatorsIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction(json)); dispatch(fetchValidatorsIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction(json)); dispatch(fetchValidatorsIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const upsertValidatorIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction(data)); dispatch(fetchValidatorsIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
...
export const deleteValidatorIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
