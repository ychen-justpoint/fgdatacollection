import {
    gotMessagetypes as gotAction,
    processingMessagetype as processingAction,
    processingMessagetypeFailed  as failedAction,
    addedMessagetype as addedAction,
    updatedMessagetype as updatedAction,
    deletedMessagetype as deletedAction,
...
const resourceUrl = process.env.REACT_APP_API + 'api/messagetypes/';
const odataResourceUrl = process.env.REACT_APP_API + 'odata/messagetypes/';
...
const myState = (state) => { return state.messagetype }
...
const returnQueryUrl = (data) => {
    ...
    if (data.hasOwnProperty('messagetypeid') && data['messagetypeid'] !== undefined && data['messagetypeid'] !== null) {
    ...
    if (data.hasOwnProperty('batchid') && data['batchid'] !== undefined && data['batchid'] !== null) {
    ...
    if (data.hasOwnProperty('jsondataid') && data['jsondataid'] !== undefined && data['jsondataid'] !== null) {
...
export const fetchMessagetypesIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction(json)); dispatch(fetchMessagetypesIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction(json)); dispatch(fetchMessagetypesIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const upsertMessagetypeIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction(data)); dispatch(fetchMessagetypesIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
...
export const deleteMessagetypeIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
