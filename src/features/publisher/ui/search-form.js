    ...
    return (
        ...
        <Form form={form} name="search-form" layout="vertical" onFinish={onFinish} initialValues={{ ...formState }}>
            <Row>
                ...
                <Col span={12}>
                    <Form.Item name="destinationid" label="Destination" rules={[{ required: false, message: 'Please select a Destination!', }]}>
                        <Select value={null}
                            showSearch
                            filterOption={(input, option) =>
                                option.items && option.items.toLowerCase().indexOf(input.toLowerCase()) >= 0
                        ...
                        >
                            <Option key={null} value={null}></Option>
                            {rest.destination.data.map((st) => (<Option key={st.id} value={st.id}>{st.name}</Option>))}
                        </Select>
                    </Form.Item>
                </Col>
                <Col span={12}>
