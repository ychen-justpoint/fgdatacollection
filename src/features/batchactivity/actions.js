import {
    gotBatchActivities as gotAction,
    processingBatchActivity as processingAction,
    processingBatchActivityFailed  as failedAction,
    addedBatchActivity as addedAction,
    updatedBatchActivity as updatedAction,
    deletedBatchActivity as deletedAction,
    updateBatchActivitySearchstate as searchAction,
...
const myState = (state) => { return  state.batchactivity[batchIdCommon.getBatchId()]}
...
export const fetchBatchActivityIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction({_batchid : batchIdCommon.getBatchId(),payload : json})); dispatch(fetchBatchActivityIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction({_batchid : batchIdCommon.getBatchId(),payload : error.message})); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction({_batchid : batchIdCommon.getBatchId(),payload : json})); dispatch(fetchBatchActivityIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction({_batchid : batchIdCommon.getBatchId(),payload : error.message})); }
        )        
...
export const upsertBatchActivityIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction({_batchid : batchIdCommon.getBatchId(),payload : data})); dispatch(fetchBatchActivityIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction({_batchid : batchIdCommon.getBatchId(),payload : error.message})); }
...
export const deleteBatchActivityIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
...
export const updateBatchActivitySearchstateIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
...
export const initBatchActivityState = (data) => (dispatch, getState) => {
    return dispatch(initBatchActivityStateAction(data)) 
}