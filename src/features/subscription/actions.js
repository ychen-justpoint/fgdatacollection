import {
    gotSubscriptions as gotAction,
    processingSubscription as processingAction,
    processingSubscriptionFailed  as failedAction,
    addedSubscription as addedAction,
    updatedSubscription as updatedAction,
    deletedSubscription as deletedAction,
...
const resourceUrl = process.env.REACT_APP_API + 'api/subscriptions/';
const odataResourceUrl = process.env.REACT_APP_API + 'odata/subscriptions/';
...
const myState = (state) => { return state.subscription }
...
const returnQueryUrl = (data) => {
    ...
    if (data.hasOwnProperty('subscriberid') && data['subscriberid'] !== undefined && data['subscriberid'] !== null && data['subscriberid'].length > 0) {

        if (filter){
            filter = filter +  ' and subscriberid eq ' + encodeURIComponent(data['subscriberid']) + ''
        }
        else
            filter = '$filter=subscriberid eq ' + encodeURIComponent(data['subscriberid']) + '' ;
    }
...
export const fetchSubscriptionsIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction(json)); dispatch(fetchSubscriptionsIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction(json)); dispatch(fetchSubscriptionsIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const upsertSubscriptionIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction(data)); dispatch(fetchSubscriptionsIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
...
export const deleteSubscriptionIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
