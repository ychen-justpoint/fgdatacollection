import {
    gotSubscribers as gotAction,
    processingSubscriber as processingAction,
    processingSubscriberFailed  as failedAction,
    addedSubscriber as addedAction,
    updatedSubscriber as updatedAction,
    deletedSubscriber as deletedAction,
...
const resourceUrl = process.env.REACT_APP_API + 'api/subscribers/';
const odataResourceUrl = process.env.REACT_APP_API + 'odata/subscribers/';
...
const myState = (state) => { return state.subscriber }
...
const returnQueryUrl = (data) => {
    ...
    if (data.hasOwnProperty('email') && data['email'] !== undefined && data['email'] !== null && data['email'].length > 0) {

        if (filter){
            filter = filter +  ' and contains(email,\'' + encodeURIComponent(data['email']) + '\')';
        }
        else
            filter = '$filter=contains(email,\'' + encodeURIComponent(data['email']) + '\')';
...
export const fetchSubscribersIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction(json)); dispatch(fetchSubscribersIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction(json)); dispatch(fetchSubscribersIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const upsertSubscriberIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction(data)); dispatch(fetchSubscribersIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
...
export const deleteSubscriberIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
