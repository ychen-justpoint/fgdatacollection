import {
    gotJsonschemas as gotAction,
    processingJsonschema as processingAction,
    processingJsonschemaFailed  as failedAction,
    addedJsonschema as addedAction,
    updatedJsonschema as updatedAction,
    deletedJsonschema as deletedAction,
...
const resourceUrl = process.env.REACT_APP_API + 'api/jsonschemas/';
const odataResourceUrl = process.env.REACT_APP_API + 'odata/jsonschemas/';
...
const myState = (state) => { return state.jsonschema }
...
const returnQueryUrl = (data) => {
    ...
    if (data.hasOwnProperty('isactive') && data['isactive'] !== undefined && data['isactive'] !== null) {

        if (filter){
            filter = filter +  ' and isactive eq ' + encodeURIComponent(data['isactive']) + ''
        }
        else
            filter = '$filter=isactive eq ' + encodeURIComponent(data['isactive']) + '' ;
    }
...
export const fetchJsonschemasIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction(json)); dispatch(fetchJsonschemasIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction(json)); dispatch(fetchJsonschemasIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const upsertJsonschemaIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction(data)); dispatch(fetchJsonschemasIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
...
export const deleteJsonschemaIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
