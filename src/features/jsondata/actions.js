import {
    gotJsondatas as gotAction,
    processingJsondata as processingAction,
    processingJsondataFailed  as failedAction,
    addedJsondata as addedAction,
    updatedJsondata as updatedAction,
    deletedJsondata as deletedAction,
...
const myState = (state) => { return state.jsondata }
...
export const fetchJsondataIfNotBusy = (data) => (dispatch, getState) => {
...
const insertObject = (data) => dispatch => {
    ...
    return fetch(resourceUrl,requestOptions)
        ...
        .then(
            (json) => { dispatch(addedAction(json)); dispatch(fetchJsondataIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
const updateObject = (data) => dispatch => {
    ...
    return fetch(url,requestOptions)
        ...
        .then(
            (json) => { dispatch(updatedAction(json)); dispatch(fetchJsondataIfNotBusy()) }
        )
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
        )
...
export const upsertJsondataIfNotBusy = (insert,data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
        if (insert)
    ...
    return fetch(url,requestOptions)
        .then(res => {
            if (!res.ok) {
                ...
                dispatch(deletedAction(data)); dispatch(fetchJsondataIfNotBusy());
            }
        })
        .catch(
            (error) => { console.log(error); dispatch(failedAction(error.message)); }
...
export const deleteJsondataIfNotBusy = (data) => (dispatch, getState) => {
    let state = myState(getState())
    if (ifNotBusy(state)) {
