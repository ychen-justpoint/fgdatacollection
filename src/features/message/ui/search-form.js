    ...
    return (
        ...
        <Form form={form} name="search-form" layout="vertical" onFinish={onFinish} initialValues={{ ...formState }}>
            <Row gutter={[8, 8]} justify={"start"} >
                <Col span={12}>
                    <Form.Item name="messagetypeid" label="Message Type" rules={[{ required: false, message: 'Please select a Type!', }]}>
                        <Select value={null} 
                                showSearch
                            ...
                            {rest.messagetype.data.map((st) => (<Option key={st.id} value={st.id}>{st.name}</Option>))}
                        </Select>
                    </Form.Item>
                </Col>
                <Col span={12}>
